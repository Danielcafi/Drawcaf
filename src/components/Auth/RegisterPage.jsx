...
export default function RegisterPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await signUp(email, password, { full_name: fullName, role: 'seller' })
      navigate('/onboarding')
    } catch (err) {
  ...
  const handleGoogleLogin = async () => {
    try {
      ...
      if (err?.message?.includes('provider is not enabled') || err?.msg?.includes('provider is not enabled') || JSON.stringify(err).includes('provider is not enabled')) {
        setError("L'inscription Google n'est pas encore activée dans votre tableau de bord Supabase (Authentication > Providers > Google). Vous pouvez créer votre compte avec votre email.")
      } else {
      ...
      >
        ...
        <div className="bg-white rounded-2xl shadow-xl p-8">
          ...
          <p className="text-center text-gray-500 mb-8">
            Commencez à vendre sur Drawcaf
          </p>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="flex items-start gap-2">
              ...
              <span className="text-sm text-gray-600">
                ...
                et la{' '}
                <Link to="/privacy" className="text-primary-100 hover:underline">
          ...
          <p className="text-center mt-6 text-gray-600">
            Déjà un compte ?{' '}
            <Link to="/login" className="text-primary-100 font-medium hover:underline">
              Se connecter
            </Link>
          </p>
        </div>
